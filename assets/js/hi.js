var hi = {
    ...
    //groupColors: ['#4575b4', '#abd9e9', '#fee08b', '#fdae61', '#d53e4f', '#DBE2E6'],
    groupColors: ['#324376','#576AA6','#f5dd90','#f9a776','#f76c5e', '#dbe2e6'],
    reversedGroupColors: ['#a00041','#cb3d4f','#ea725f','#fba871','#ffe185','#DBE2E6'],
    //reversedGroupColors: ['#F7EFED', '#DDD2CE', '#ABA1A0', '#68656E', '#2A3249', '#DBE2E6'],
    outlierColor: '#5BC0EB',
    ...
    indicatorCodes: ['Median_household_income', 'LowBirthw', 'BirthsTeens', 'PreBirths', 'Pop_wNo_Health_Ins', 'Hospital_Amount', 'Emergency_Amount'],
    ...
    //currentIndicatorId: 'weight',
    currentIndicatorCode: 'Median_household_income',
    //currentIndicatorCode: 'LowBirthw',
    currentIndicatorName: 'Median Household Income',
    //currentIndicatorName: 'Low Birthweight Births',
    currentMapType: 'grid',
    ...
    init: function(){
        ...
        $(document)
            ...
            .ajaxComplete(function() {
                $("#page-loader").delay(1000).fadeOut(300);
            });

        //selectize
        $.each($('.selectbox'), function(){
            var $this = $(this);
            _this.selectList[$this.attr('id')] = $this.selectize({
                create: true
            });
        });

        //switch
        $('.show-tooltip').lc_switch();
        $('.show-tooltip').lcs_on();


        //tooltip
        $('.chart-title .icon-info').tipsy({
            gravity: 's'
        });
    ...
    addEventListener: function(){
        ...
        $('#indicator-select').on('change', function(e){
            ...
            if(_this.currentDataType == 'pct'){
                ...
                _this.currentIndicatorCode = _this.currentIndicatorCode.replace('Pct_', '');
            }
        ...
        //hide or show toolip
        $('.chart-component').delegate('.show-tooltip', 'lcs-on', function() {
            $(this).closest('.chart-component').find('.tooltip').show();
        });

        $('.chart-component').delegate('.show-tooltip', 'lcs-off', function() {
            $(this).closest('.chart-component').find('.tooltip').hide();
        });
    ...
    createRangeSlider: function(){
        ...
        //update title
        $('#slider-title').html(template('slider-title', {indicatorName: _this.currentIndicatorName, indicatorId: _this.currentIndicatorId, unit: unit}));
    ...
    updateRangeSlider: function(){
        ...
        var threshold = _this.currentGroupThreshold;
        var unit = _this.process.getUnit();
        var postfix = _this.currentDataType === 'num' ? '' : '%';
