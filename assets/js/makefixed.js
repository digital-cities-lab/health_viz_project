...
$.fn.makeFixed = function (options)
{
	...
	$(defOpts.container).scroll(function(e)
	{

		/* We get the current scroll position */

		var scrolled = $(defOpts.container).scrollTop();
		...
		$(sel).each(function()
		{
			...
			if (el.attr(attr.fAtPos) <= scrolled + defOpts.defTopPos && scrolled > 0)
			{
				...
				({
					...
					top:        (el.attr(attr.fTopPos)) ? el.attr(attr.fTopPos) + 'px' : defOpts.defTopPos + 'px',
